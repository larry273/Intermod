...
private:
	...
	//variables:
	int order = 0,  step = 0, n = 0, incrementer_one = 0;
	float range_frequencies = 0;
...
public:
	...
	void run(int order, float range, std::vector<float> rxFreq, std::vector<float> txFreq); //run function

	//vectors:
	std::vector<std::string> equation_vec;
	std::vector<float> result_vec; //vector that will hold all results
	std::vector<std::vector<float> > result_vec_2d; //receiver frequencies it hits
