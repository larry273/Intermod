...
QString OutputClass::createOutputHTML(std::vector<float> resultList, std::vector<std::string> eqList, std::vector<float> rxList, std::vector<float> txList, std::vector<QString> nameList, int order, float range, std::vector<int> orderVector) {
...
void OutputClass::setHTML() {
	...
	QString html;
	html = OutputClass::createOutputHTML(rList, eq, this->rxVector, this->txVector, this->nVector, this->maxOrder, this->calcRange, indexR);

	//draw graph to base64 and set into html
	QString graph;
	graph = OutputClass::drawGraph(rList, this->rxVector, this->txVector, this->nVector);
